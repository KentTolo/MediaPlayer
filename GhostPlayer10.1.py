...
class MediaPlayer(QWidget):
    def __init__(self):
        ...
        self.setGeometry(200, 200, 900, 600)  # Increased height for more space
        self.init_ui()
    ...
    def init_ui(self):
        ...
        right_layout.addWidget(self.video_label, 1)  # Allow video label to take more space
    ...
    def load_media(self, media_path):
        ...
        if media_path.endswith(('.mp4', '.avi')):
            try:
                ...
                self.timer.start()
                self.update_video_frame()
                # Extract and play video audio
                audio_file = self.media_clip.audio
                if audio_file:
    ...
    def update_progress(self):
        if self.playing:
            if self.media_clip:  # For video
                current_time = self.media_clip.reader.pos / self.media_clip.fps
    ...
    def update_video_frame(self):
        if self.playing and self.media_clip:
            try:
                current_time = self.media_clip.reader.pos / self.media_clip.fps
                frame = self.media_clip.get_frame(current_time)
    ...
    def set_position(self, position):
        if self.playing and self.media_clip:
            new_time = position
            self.media_clip.reader.seek(new_time)
            self.update_video_frame()
