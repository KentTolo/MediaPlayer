...
def update_progress():
  ...
  while True:
    if pygame.mixer.music.get_busy() and not paused:
      ...
      # Check if the current song has reached is maximum duration
  ...
  if selected_folder_path:
    ...
    for filename in os.listdir(selected_folder_path):
      if filename.endswith(".mp3"):
        lbox.insert(tk.END, filename) # Insert only the filename , mot the full path 
...
def play_selected_song():
  ...
  if len(lbox.curselection()) > 0 :
    ...
    pygame.mixer.music.play(start=current_position) # PLay song from the current position
    paused = False
    audio = MP3(full_path)
...
def stop_music():
  global paused
  # Stop the current playing music and reset the progress bar
...
#create a label for media player
l_music_player = tk.Label(window, text="Media Player", font = ("TkDefaultFont", 30, "bold"))
...
#create button to go to previous song
