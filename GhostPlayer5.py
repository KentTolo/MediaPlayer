...
class MediaPlayer(QWidget):
    ...
    def load_media(self, media_path):
        ...
        if media_path.endswith(('.mp4', '.avi')):
            ...
            try:
                ...
                self.playing = True
                self.current_media_label.setText(f"Playing: {os.path.basename(media_path)}")
                self.progress_slider.setMaximum(int(pygame.mixer.Sound(media_path).get_length()))
            except Exception as e:
    ...
    def update_video_frame(self):
        if self.playing and self.media_clip:
            try:
                frame = self.media_clip.get_frame(self.media_clip.reader.pos / self.media_clip.fps)
                if frame is not None:  # Ensure the frame is valid
                    ...
                    QTimer.singleShot(33, self.update_video_frame)
            except Exception as e:
                print(f"Error updating video frame: {e}")

    def set_position(self, position):
        if self.media_clip:
            self.media_clip.reader.seek(position / self.media_clip.fps)
